...
const RUNTIME_MODULE_NAME = 'myst-anywidget-static-host.mjs';
...
function findOutputsWithParent(tree) {
  ...
  function visit(node, parent) {
    ...
    if (Array.isArray(node)) { for (const n of node) visit(n, parent); return; }
...
// Scan the top-level widget-state map for `LinkModel`/`DirectionalLinkModel` entries
// produced by `widgets.jslink((src, attr), (tgt, attr))` and `widgets.jsdlink(...)`.
// These are siblings — not descendants — of the cell-output view roots, so the
// `buildSubModels` BFS will never find them. We surface them here so the plugin can
// synthesize a client-side binding without a connector widget.
...
// Shared static runtime used by generated wrapper modules. This keeps the user's
// original ESM untouched and shapes the browser-side glue around anywidget AFM
// concepts: AnyModel, a scoped registry, and host.getModel/host.getWidget.
const RUNTIME_SOURCE = String.raw`// Generated by plugins/anywidget-static-export.mjs.
...
function __mystNormalizeRef(ref) {
...
class __MystSubModel {
  constructor(state, buffers) {
    ...
    this._events = new __MystEmitter();
  }
...
function __mystRegistry() {
  ...
  var scope = document && document.baseURI ? document.baseURI : 'default';
  if (!window.__myst_anywidget_hosts.has(scope)) {
    window.__myst_anywidget_hosts.set(scope, __mystCreateRegistry());
  }
...
function __mystSetupModel(model) {
  ...
  //     them on the instance. save_changes/send are no-ops with no kernel.
  ...
  reg.register(model, __mystKeysForState({
...
function __mystNormalizeModule(mod) {
  var candidate = mod && mod.default !== undefined ? mod.default : mod;
  if (typeof candidate === 'function') return candidate;
  return candidate || {};
}

export async function initializeStaticWidget(userModule, args) {
  var widget = __mystNormalizeModule(userModule);
  __mystSetupModel(args.model);
  var nextArgs = Object.assign({}, args, { host: __mystHost() });
  if (typeof widget === 'function') {
    widget = await widget(nextArgs);
  }
  var exports = undefined;
  if (widget && typeof widget.initialize === 'function') {
    var result = await widget.initialize(nextArgs);
    if (result && typeof result === 'object') exports = result;
  }
  var rootId = args.model && args.model.get && args.model.get('_myst_root_id');
  var widgetId = args.model && args.model.get && args.model.get('widget_id');
  var anywidgetId = args.model && args.model.get && args.model.get('_myst_anywidget_id');
  var reg = __mystRegistry();
  reg.registerBinding(args.model, {
    keys: __mystKeysForState({ widget_id: widgetId, _anywidget_id: anywidgetId }, rootId),
    exports: exports,
    render: function (opts) {
      return renderStaticWidget(widget, Object.assign({}, opts, {
        model: args.model,
      }));
    },
  });
...
export async function renderStaticWidget(userModule, args) {
  var widget = args.model && args.model.__mystUserWidget
    ? args.model.__mystUserWidget
    : __mystNormalizeModule(userModule);
  __mystSetupModel(args.model);
  if (args.model && args.model.get) {
    __mystEnsureShadowCss(args.el, args.model.get('_myst_css_text'), args.model.get('_myst_css_key'));
  }
  var nextArgs = Object.assign({}, args, { host: args.host || __mystHost() });
  if (typeof widget === 'function') {
    widget = await widget(nextArgs);
...
// have no view in upstream either.
const JSLINK_RUNTIME_SOURCE = `// Generated by plugins/anywidget-static-export.mjs.
// Resolves jslink/jsdlink bindings collected from the notebook's widget-state.
...
function suppressJslinkReprs(tree) {
  ...
  for (const { node, parent } of findOutputsWithParent(tree)) {
    ...
    const text = Array.isArray(raw?.content) ? raw.content.join('') :
